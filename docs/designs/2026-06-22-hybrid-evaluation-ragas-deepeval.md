...
| 7 | 数据落库 | **落库与否为 §4-B 待决策项**（核实发现 `EvalStore` 纯内存、`EvalTable` 全项目零读写，见 §4-B「前提事实修正」）。若决定落库：复用 `EvalTable` 既有 `ragas_*` 列（迁移 `d48d7137f57f` 已建三列、`20260623` 已补 `ragas_context_recall`），Tutor 指标走 `eval_data` JSON 列，不新建列 |
...
| `ComponentBench` | L2 eval | 跑黄金用例、按 expected 阈值判 passed | `EvalKernel` | `app/eval/component_bench.py:4`（`run` 在 :11） | 🟢 保持（Phase 1）/ 🟡 改（Phase 3 加分阶段诊断） |
...
| context_recall 列已建但 evaluate 未算（缺 ground_truth） | `app/models/tables.py:55` |
...
> **现状说明（避免 Phase 3 误解）**：当前 `retriever.py:181` 调用 `build_judge(target_agent_family="anthropic")` 是**硬编码常量**，并非动态推断被评 Agent 的真实模型族。即「不同族校验通过」是因为传入的常量恰与 judge 族（OpenAI）不同，而非系统真的探测了被评模型。Phase 3 接 Tutor G-Eval 时若需按被评 Agent 实际模型动态选族，须改造此处传参，不能假设 `build_judge` 会自动推断。
...
| 启发式 | recall_at_k、redundancy、latency_ms | 无 LLM（成本为零）| ✅ 始终保留 |
...
- `app/agents/retriever.py:169-265` — RAGAS 三件套替换字符 Jaccard；recall_at_k/redundancy 保留启发式；4 层降级（judge=None → 无 golden_answer → ImportError → Exception）
- `app/models/tables.py:55` — 补 `ragas_context_recall = Column(Float, nullable=True)`
...
- 诊断逻辑：对非 degraded 的 retriever 结果取平均分；依次检查 context_recall < 0.8（建议加 top_k/优化分块）/ context_precision < 0.8（建议加 reranking）/ faithfulness < 0.9（建议强化 Tutor prompt）/ **answer_relevancy < 0.8（建议检查 user_input 理解）**
  ...
  - ⚠️ **§3.2 语义修正的影响传播（不止诊断阈值一处）**：answer_relevancy「恒高、信息量低」这一修正，下游有**两个独立使用点**，Phase 3 须一并处理：
    ...
    - **passed 判定阈值**（易漏）：`golden_cases.py:19-20` retriever 用例的 `expected={"answer_relevancy": 0.6, ...}`，是 `ComponentBench.run()` 的 **通过/失败判定**依据（`component_bench.py:31-43` 逐项比 `actual < threshold`，比较在 :37），与诊断阈值是两处独立代码路径。恒高指标配 0.6 阈值 → 永远 pass、丧失判别力。Phase 3 调整 answer_relevancy 时，**必须同步改/移除此 expected 阈值**，否则诊断改了、判定仍失真。
...
- Tutor 指标走 `EvalTable.eval_data`（JSON 列，`tables.py:56`），不新建列（若 §4-B 决定落库）
...
- **依赖构造（易漏，经核查 `api/eval.py` 当前无 EvalKernel 实例）**：4-A 需自建评估栈——
  - 构造 `agent_map = {"retriever": RetrieverAgent(coordinator=...)}`（`RetrieverAgent.__init__(self, coordinator: RAGCoordinator | None = None)`，构造注入，已核查 `retriever.py:31-32`）
...
- **RAG 后端：强制 pgvector，否则整体 degraded（决策：选 A，经核查）**：`RAGCoordinator` 默认后端是 `fake`（`settings.rag_backend = "fake"`，`config.py:21`；生产 `assembly.py:121` 的 `RetrieverAgent()` 未注入 coordinator，走默认 fake）。4-A 构造 RetrieverAgent 时**必须显式注入一个 `rag_backend=pgvector` 的 RAGCoordinator**；若环境未配 pgvector（`settings.rag_backend != "pgvector"` 或 pgvector 不可用）→ **rerun 整体走 degraded 路径**（与 judge=None 同构），不跑 RAGAS、不给假分。
  - 选型判据（穷举三选项）：
    | 选项 | 做法 | 代价 |
    |---|---|---|
    | **A（选它）** 强制 pgvector，否则 degraded | 检查 `settings.rag_backend`，非 pgvector 即整体 degraded | rerun 环境须配齐 pgvector + embedding key |
    | B 沿用 settings（默认 fake） | `RetrieverAgent()` 自建，读 settings | **fake 后端下 RAGAS 对假检索结果打分，评估无意义**——正是 §1 要避免的"假分" |
    | C rerun 请求带 backend 参数 | API 入参显式传后端 | 契约变复杂，调用方要懂后端语义 |
  - → 选 A：RAGAS 三件套的全部意义是评估**真实检索质量**，fake 后端会让 faithfulness/precision 变成"对假数据打分"；A 把"无 pgvector"明确归入 degraded（诚实、与既有降级语义一致），B 是"看起来能跑就停"的自洽陷阱。
...
- **聚合逻辑（决策：选 1 求平均）**：`run_component_bench` 返回 `list[EvalResult]`（每条黄金用例一个），但 `rerun_eval` 的 `response_model` 是**单个** `EvalResponse`（`api/eval.py:30`）。聚合规则：
  ...
  - **`mastery_score` / `mastery_level` 一律置 None（决策：选 A，经核查）**：`EvalResponse` 含 8 字段，除 4 个 ragas_* 外还有 `mastery_score` / `mastery_level`（`app/models/schemas.py:42-43`）。这两字段属 **Critic 的学习者掌握度领域**，与 Retriever 检索质量**正交**——`Retriever.evaluate()` 返回的 dict 里根本没有 mastery（已核查，核心 key 为 faithfulness/answer_relevancy/context_precision/recall_at_k/redundancy/latency_ms/degraded；降级路径额外返回 `degraded_reason`）。且**全项目无任何代码向 `EvalStore` 写过 mastery**：mastery 的在线来源是 `CriticAgent.handle()` 发射的 `MASTERY_ASSESSED` 事件，经 `extract_mastery_score(events)`（`app/orchestration/assembly.py:44-50`）提取后填入 `ChatResponse`（`app/api/chat.py`），这条链**不经过 `EvalStore`**；`api/eval.py` 的 GET/rerun 取 mastery 只能拿到默认 `0`/`""`。所以「复用 session 已有 mastery」前提不成立。
    - **⚠️ 选 A 的前置改动（经核查，易漏）**：`schemas.py:42-43` 当前是 `mastery_score: int` / `mastery_level: str`，**均非 Optional**——直接填 `None` 会触发 Pydantic `ValidationError`。故选 A **必须先把这两字段宽松化**为 `mastery_score: int | None = None` / `mastery_level: str | None = None`，再在 `_aggregate_results` 填 None。这是向后兼容的宽松化（既有写 int/str 的路径不受影响），但属对 schema 契约的真实改动，不可当作"零改动"。
    - 选型判据（穷举三选项）：
      | 选项 | 做法 | 代价 |
      |---|---|---|
      | **A（选它）** mastery 标 None | schema 两字段改 Optional + 聚合填 None | 改 schema（宽松化，向后兼容）；retriever rerun 场景 mastery 半边恒 None |
      | B 复用 session mastery | 先实现"在线 mastery 落 EvalStore"再读回 | **前提不成立**：当前无任何代码落过 mastery；且引入跨维度持久化 feature，超出本 spec scope |
      | C 部件评估专用 response | 新建 `ComponentEvalResponse`（只含 ragas+degraded）替代 EvalResponse | 改 `rerun_eval` 的 response_model，牵连前端契约，留待 Phase 3 接 Tutor/Critic bench、评估维度增多时再上 |
    - → 选 A：检索质量与学习掌握度正交，retriever bench 本不产 mastery；A 的改动（schema 宽松化）仍小于 B（造持久化）/C（改 response 契约+前端），且 `None` 比 `0` 诚实。
  - 关键函数：`_aggregate_results(results: list[EvalResult]) -> dict`（取非 degraded 子集求均值；mastery 两字段恒 None；返回映射前的指标 dict）
- 用例来源决策（待定，不可臆测）：
...
Retriever.evaluate() 返回的 dict key 与 EvalTable 列名、EvalResponse 字段不完全一致，需显式映射。**改名发生位置（经核查）**：`EvalStore.save()` 只做 `{"session_id": ..., **eval_data}` 透传、**不改任何 key 名**（`eval_store.py`）；因此 `faithfulness→ragas_faithfulness`、`answer_relevancy→ragas_relevancy` 的改名**必须在 4-A 的聚合/映射代码里显式做**（一个映射字典），不能依赖 save() 自动改名。仅列出**有明确落点**的 RAGAS 核心指标：

| evaluate() 返回 key | EvalTable 列名 | EvalResponse 字段 | 映射逻辑位置 |
|---|---|---|---|
| `faithfulness` | `ragas_faithfulness` | `ragas_faithfulness` | 4-A 映射字典（save() 不改名） |
| `answer_relevancy` | `ragas_relevancy` | `ragas_relevancy` | 同上（改名，非同名透传） |
...
**不在映射表内的辅助指标**：`recall_at_k`、`redundancy`、`latency_ms` 三个启发式指标由 Retriever.evaluate() 返回，但 **EvalTable 无对应列、EvalResponse 无对应字段**（经代码核查 `tables.py:46-57` / `app/models/schemas.py:39-47`）。去向：
...
- **mastery 字段（P1 选 A）**：`schemas.py:42-43` 两字段已宽松化为 `int | None` / `str | None`；rerun 返回的 `mastery_score`/`mastery_level` 为 `None`（非 0/""），且 Pydantic 不报 ValidationError
- **RAG 后端（P2 选 A）**：`settings.rag_backend == "pgvector"` 时跑真实 RAGAS、返回非 degraded 真分；`!= "pgvector"`（默认 fake）时 rerun 整体返回 degraded，不产生假分
...
| **RAG 后端非 pgvector（仅 §4-A rerun 路径）** | `settings.rag_backend != "pgvector"` | 整体 degraded，不跑 RAGAS（fake 检索结果打分无意义，见 §4-A P2 决策）|
